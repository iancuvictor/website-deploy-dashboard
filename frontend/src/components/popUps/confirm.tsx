...
export default function Confirm({ message, confirmText, denyText, confirmFn, denyFn }: ConfirmProps) {
    ...
    flex items-center justify-center bg-black/80">
        <div className={`${darkMode ? 'bg-neutral-950 text-white' : 'bg-white text-black'} h-80 min-w-100 max-w-140 p-10 ring-1 ring-neutral-700 rounded-md
        flex flex-col gap-10 h-fit`}>
            <span>{message}</span>
            <div className="flex justify-between">
                <button onClick={() => confirmFn()}
                    className="cursor-pointer bg-rose-500 hover:bg-rose-600 min-w-30 p-2 rounded-xs text-white">{confirmText}</button>
