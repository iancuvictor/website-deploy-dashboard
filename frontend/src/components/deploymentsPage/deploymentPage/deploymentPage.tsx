...
import { faCircleNotch, faPlus, faX } from "@fortawesome/free-solid-svg-icons";
...
export default function DeploymentPage() {
    ...
    const [form, setForm] = useState<Form>(deploymentData)
    ...
    const updateDeploymentData = useMutation({
        mutationFn: (form: Form) => axios.put(`${API_URL}/api/deployments/deployment?id=${id}`, form),
        onSuccess: () => {
    ...
    if (error) {
        ...
        if (isLoading || form === undefined) {
            ...
            return <div className={`${darkMode ? 'text-white' : 'text-black'} absolute flex flex-col gap-10 h-full w-full p-10`}>
                <div className="flex flex-col items-center justify-center">
                    <h1 className="text-[30px] font-[700]">{form.name}</h1>
                    <a href={deploymentData.deploymentUrl} target="_blank"
                        className="cursor-pointer text-[16px] text-blue-400 hover:underline underline-offset-1">{form.deploymentUrl}</a>
                </div>
                <div className="flex flex-col gap-4 p-5 rounded-xs ring-1 ring-neutral-700 w-200">
                    {/* <span 
                    className={`${currentStatus === 'running' ? 'text-green-500' : 'text-white'}`}>
                        Website is running</span> */}
