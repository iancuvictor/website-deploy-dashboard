import { faFileArrowDown, faFloppyDisk, faFolderOpen } from "@fortawesome/free-solid-svg-icons";
...
export default function Logs({ stepId, logsPath }: LogProps) {
    ...
    useEffect(() => {
        const handleLog = (data: { stepId: string; chunk: string }) => {
                setLogs((prev) => [...prev, {stepId: data.stepId, message: data.chunk}]);
        };
    ...
    return <div className="flex flex-col gap-3 w-205 h-100 ring-1 ring-neutral-700 p-5">
        <div className="w-full flex items-center gap-5">
            ...
            <div className="w-full flex flex-row gap-2">
                ...
                <button className="cursor-pointer ring-1 ring-neutral-700 p-2 hover:bg-neutral-900 whitespace-nowrap">
        ...
        <div className="p-2 ring-1 ring-neutral-700 h-full whitespace-pre-wrap overflow-y-scroll">
            {[...logs].filter((log) => log.stepId === stepId).map((log) => {
